...
test("buildRunResponseText should format fund output as dashboard-style sections", () => {
  ...
  assert.match(text, /核心结论: 持有。保持仓位，等待趋势确认。/);
  assert.match(text, /数据视角: 数据完整性=完整；最新值=基金 4\.28；短线回报=1日\+0\.45%\/5日\+1\.56%；中期回报=20日\+1\.23%\/60日\+3\.45%\/120日\+6\.78%；风险=回撤-2\.8%\/波动11\.2%\/修复8天；相对表现=同类分位88\.2\/20日分位变化\+6\.4\/60日分位变化\+10\.8\/同类排名18\/240；信号概览=信号偏积极，证据支撑尚可/);
  assert.match(text, /情报观察: 风险提示=波动率抬升；公告\/提示=披露季度报告；基金经理变化=基金经理分工调整；现任基金经理=张三；李四；重仓参考=贵州茅台\(9\.80%\)；宁德时代\(8\.12%\)/);
  assert.match(text, /执行计划: 持仓者继续持有；未持仓者暂不追高；仓位处理=维持仓位；规则约束=当前不宜做买入\/需要留意存在申购赎回限制；持仓背景=当前持仓100\/成本4\.2\/估算市值428\/估算盈亏\+1\.9%\/预算1000\/风险偏好均衡\/周期中期/);
