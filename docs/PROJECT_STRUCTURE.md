...
- `src/integrations/market-analysis/` 现按能力分层：`fund/` 放基金分析主链路，`equity/` 放股票/指数旧链路与公开新闻上下文，`reporting/` 放 markdown 报告与长图输出 adapter，root 只保留命令入口、运行时编排、格式化与存储。
