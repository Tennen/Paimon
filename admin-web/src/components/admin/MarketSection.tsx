    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-3 rounded-md border border-border p-3">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="market-fund-news-query-suffix">基金新闻 querySuffix</Label>
            <Input
              id="market-fund-news-query-suffix"
              value={props.marketAnalysisConfig.fund.newsQuerySuffix}
              onChange={(event) => props.onMarketFundNewsQuerySuffixChange(event.target.value)}
              placeholder="基金 公告 经理 申赎 风险"
            />
            <p className="text-xs text-muted-foreground">
              Search Engine 是全局配置；这里仅配置基金分析场景下的新闻检索补充关键词。
            </p>
          </div>
